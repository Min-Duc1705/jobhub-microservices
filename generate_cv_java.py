...
def build_unique_names(count=50):
    ...
    existing_slugs = set()
    for folder in ["CV", "CV-1", "CV NET", "CV java"]:
    ...
    # 3. Shuffle with seed 2028 (fresh diversity and uniqueness)
    random.seed(2028)
...
def main():
    output_dir = os.path.join("T:\\TryHard_IT_Project\\Final\\Backend", "CV java")
    ...
    if os.path.exists(output_dir):
        try:
            ...
            print(f"[Warning] Failed to delete existing CV java folder: {e}")
    os.makedirs(output_dir, exist_ok=True)
    
    print(f"=== BẮT ĐẦU SINH 50 CV BACKEND JAVA TRONG THƯ MỤC: {output_dir} ===")
