...
def main():
    cv_dir = "t:/TryHard_IT_Project/Final/Backend/CV"
    # Get all candidate CV files, excluding original templates
    files = sorted([
        f for f in os.listdir(cv_dir) 
        if f.startswith("cv_") and f.endswith(".pdf")
    ])
    
    print(f"=== BẮT ĐẦU SEED {len(files)} ỨNG VIÊN TỪ THƯ MỤC CV ===")
    ...
    for i, filename in enumerate(files, 1):
        ...
        # 1. Parse name slug and check if already registered
        auth_cur.execute('SELECT "Id" FROM "AppUsers" WHERE "Email" = %s', (name_slug + "@gmail.com",))
        # Wait, since the email from the PDF could be gmail, yahoo, or outlook, we check the database for the exact email if we can extract it first.
        # But to check if it's already seeded, let's extract PDF info first!
        try:
        ...
        # 2. Check if user already exists in AuthDb
        user_id = None
        auth_cur.execute('SELECT "Id" FROM "AppUsers" WHERE "Email" = %s', (email,))
        row = auth_cur.fetchone()
        if row:
            user_id = row[0]
            # 3. Check if CV is already uploaded for this user
        ...
        # 4. Register candidate account if not exists
        if not user_id:
        ...
        # 5. Activate candidate account in AuthDb
        try:
        ...
        # 6. Wait for profile to be created via MassTransit and update Phone, Address
        try:
            found_profile = False
        ...
        # 7. Login to obtain Access Token
        token = None
        ...
        # 9. Create Resume record to link CV
