...
def main():
    cv_dir = "t:/TryHard_IT_Project/Final/Backend/CV tester"
    ...
    print(f"=== BẮT ĐẦU SEED {len(files)} ỨNG VIÊN TỪ THƯ MỤC CV TESTER ===")
    ...
    for i, filename in enumerate(files, 1):
        ...
        # Calculate experience years based on level
        exp_years = 0
        ...
        print(f"[{i}/{len(files)}] Đang xử lý: {name} | {email} | {phone} | {address} | Level: {level} ({exp_years} YOE)...")
        ...
        try:
            ...
            if not found_profile:
                profile_cur.execute('''
                    INSERT INTO "Customers" ("Id", "AppUserId", "Type", "FullName", "Phone", "Address", "YearsOfExperience", "CreatedDate", "CreatedBy", "IsDeleted")
                    VALUES (%s, %s, 'CANDIDATE', %s, %s, %s, %s, NOW(), 'Seeder', false)
                ''', (user_id, user_id, name, phone, address, exp_years))
                profile_conn.commit()
                ...
                profile_cur.execute('''
                    ...
                    WHERE "AppUserId" = %s
                ''', (phone, address, exp_years, user_id))
                profile_conn.commit()
    ...
    print(f"\n=== KẾT QUẢ SEEDING THƯ MỤC CV TESTER ===")
