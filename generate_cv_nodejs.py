...
def build_unique_names(count=50):
    ...
    existing_slugs = set()
    for folder in ["CV", "CV-1", "CV NET", "CV java", "CV Python", "CV NodeJS"]:
    ...
    # 3. Shuffle with seed 2030 (fresh diversity and uniqueness)
    random.seed(2030)
...
def main():
    output_dir = os.path.join("T:\\TryHard_IT_Project\\Final\\Backend", "CV NodeJS")
    ...
    if os.path.exists(output_dir):
        try:
            ...
            print(f"[Warning] Failed to delete existing CV NodeJS folder: {e}")
    os.makedirs(output_dir, exist_ok=True)
    
    print(f"=== BẮT ĐẦU SINH 50 CV BACKEND NODEJS TRONG THƯ MỤC: {output_dir} ===")
    
    levels = ["INTERN", "FRESHER", "JUNIOR", "MIDDLE", "SENIOR", "LEADER", "MANAGER"]
    frameworks = ["Node.js", "Express.js", "NestJS"]
    
    # Seed random for repeatability inside this run
    random.seed(2030)
    ...
    for i in range(50):
        ...
        # Randomly choose NodeJS / Express / NestJS in title and specialty
        profile["specialty"] = f"Backend {framework}"
        profile["title"] = f"{framework} Developer"
        
        # 5. Define Output Path
        name_slug = make_slug(full_name).replace(".", "_")
        spec_slug = "backend_node.js"
