...
def build_unique_names(count=50):
    # 1. Gather all name slugs from existing CV, CV-1 and CV NET folders to exclude them
    existing_slugs = set()
    for folder in ["CV", "CV-1", "CV NET"]:
        folder_path = os.path.join("T:\\TryHard_IT_Project\\Final\\Backend", folder)
        if os.path.exists(folder_path):
            for f in os.listdir(folder_path):
                if f.startswith("cv_") and f.endswith(".pdf"):
                    # Filename format: cv_bui_xuan_trang_backendnet_middle.pdf
                    # We want to get the name slug which is the first few words after cv_
                    parts = f[3:-4].split("_")
                    if len(parts) >= 3:
                        # Reconstruct the name slug (usually 3 parts for Last Middle First)
    ...
    # 3. Shuffle with seed 2027 (new seed to avoid any overlaps and get fresh diversity)
    random.seed(2027)
...
def main():
    output_dir = os.path.join("T:\\TryHard_IT_Project\\Final\\Backend", "CV NET")
    ...
    if os.path.exists(output_dir):
        try:
            ...
            print(f"[Warning] Failed to delete existing CV NET folder: {e}")
    os.makedirs(output_dir, exist_ok=True)
    
    print(f"=== BẮT ĐẦU SINH 50 CV BACKEND .NET TRONG THƯ MỤC: {output_dir} ===")
    
    levels = ["INTERN", "FRESHER", "JUNIOR", "MIDDLE", "SENIOR", "LEADER", "MANAGER"]
    
    # We want 50 Backend .NET CVs
    configs = []
    for j in range(50):
